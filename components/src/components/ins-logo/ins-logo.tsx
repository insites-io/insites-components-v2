...
@Component({ tag: 'ins-logo' })
export class insLogo {
  @Element() insLogo: HTMLElement;
  ...
  componentDidLoad () {
    this.modal = this.insLogo.querySelector('.modal');
    this.hiddeninput = this.insLogo.querySelector('.hidden-input');
  }

  initCropper() {
    let component = this;
    this.image = this.insLogo.querySelector('.image');
  ...
  displayImage(evt) {
    ...
    if (FileReader && files && files.length) {
        ...
        fr.onload = function () {
            component.ImageElement = component.insLogo.querySelector('.image');
            // component.ImageElement.src = fr.result;
            component.insLogo.querySelector('.upload').classList.remove('show');
            component.insLogo.querySelector('.save').classList.add('show');
  ...
  exportImage(e) {
    e.preventDefault();
    this.ImageElement = this.insLogo.querySelector('.profile');
    this.ImageElement.src = this.base64;
    this.insLogo.querySelector('.img-placeholder').classList.remove('show');
    ...
    return (
      <div class="ins-logo-wrapper">
        <div class="image-picker">
          ...
          <div class="inline-block">
            <span>Transparent PNG or SVG at least 300px</span>
