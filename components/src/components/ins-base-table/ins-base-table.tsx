...
@Component({ tag: 'ins-base-table' })
export class InsBaseTable {
  @Element() insBaseTableEl: HTMLElement;

  @Event() onPaginationChange: EventEmitter;
  @Event() onTableSearch: EventEmitter;
  @Event() onTableSort: EventEmitter;
  @Event() onTableBulkAction: EventEmitter;
  @Event() onTableRowAction: EventEmitter;
  @Event() onFieldChange: EventEmitter;
  ...
  componentDidUpdate(){
    if (this.tableUpdated){
      let tdCheckboxes = this.insBaseTableEl.querySelectorAll('ins-checkbox') as any;
      let insCheckboxAll = this.insBaseTableEl.querySelector('#checkAll') as any;
      let insCheckboxAllAccordion = this.insBaseTableEl.querySelector('#checkAllAccordion') as any;
  ...
  onSearchHandler(event) {
    this.onTableSearch.emit(event.detail);
  }

  @Listen('onClickInsButton')
  onClickInsButtonHandler(event){
    if (event.target.id === 'insBaseTableBulkButton'){
  ...
  onCheckInsCheckbox(event){
    ...
    if (event.detail.value === 'checkAll'){
      ...
      let checkAllEl = this.insBaseTableEl.querySelector('#checkAll') as any;
  ...
  bulkActionHandler(){
    let bulkActionEl = this.insBaseTableEl.querySelector('ins-select[data-type="bulk-action"]') as any;
  ...
  rowActionHandler(action, data, header){
    this.onTableRowAction.emit({ action, header, data });
  }
          ...
          <div class="ibt-table-header-wrap">
            ...
            {this.withoutSearch ? '' : // || this.loadingScreen
              <div class={`ins-searchbar-container ${this.searchPosition === 'left' ? 'left' : ''}`}>
                <ins-searchbar placeholder={this.searchbarPlaceholder} value={this.initialSearch}></ins-searchbar>
              </div>
            }
          </div>
        : ''}
