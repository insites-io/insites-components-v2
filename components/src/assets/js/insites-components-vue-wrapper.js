var templateBuilder = function(componentKey, eventsList) {
  ...
  var eventPairsString = eventPairs.map(function([camelCasedEventString, kebabCasedEventString]) {
    ...
    if (camelCasedEventString === "insValueChange"){
      return `@${ camelCasedEventString }="inputHandler"`
    }

    return `@${ camelCasedEventString }="$emit('${ camelCasedEventString }', $event);"`
  }).join(' ');
...
var componentDetailsMapping = {
  ...
  // 'drawer':           ['toggleDrawer'],
  'editor':           ['insUpload', 'insInput', 'insBlur', 'insValueChange'],
  // 'filter':           ['onFilterApply'],
  // 'heading':          ['onInsHeadingUpdate'],
  'image-picker':     ['insValueChange'],
  ...
  'select':           ['insOptionSelect', 'insSubmit', 'insSearch', 'insClose', 'insLoadMore', 'insValueChange'],
  // 'sort':             ['onDragStart', 'onDragEnd', 'onChoose', 'onAdd', 'onUpdate', 'onMove', 'onRemoved', 'onCloned', 'onPositionChanged'],
  // 'tab-item':         ['tabItemError'],
  // 'tab':              ['onchangeTab'],
  'textarea':         ['insInput', 'insValueChange'],
  // 'timeline':         ['paginationChange'],
  // 'timepicker':       ['onblur', 'oninput', 'valueChange'],
  'toggle-switch':    ['insToggle', 'insValueChange'],
  // 'modal':            ['onInsModalClose'],
  'input-slider':     ['insSlide', 'insSlideStart', 'insSlideEnd', 'insValueChange'],
  'input-stepper':    ['insBlur', 'insValueChange'],
  // 'accordion':        ['toggle'],
  // 'accordion-item':   [],
  // 'accordion-link':   [],
  // 'admin':            [],
